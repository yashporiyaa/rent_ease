...
export function generateMinimalTemplate(
  ...
  if (data.customer.phone) {
    doc.fontSize(10).text(`Phone: ${data.customer.phone}`, margin, sectionTop + 40);
  }
  ...
  const unitX = margin + contentWidth * 0.70;
  ...
  doc.text(
    `Rs. ${data.totals.grandTotal}`,
    totalX,
    totalsTop + 50,
    { width: 70, align: 'right' },
  );

  doc.moveTo(unitX, totalsTop + 70)
